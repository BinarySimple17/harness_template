#!/bin/bash
# init.sh — стандартный путь инициализации и верификации harness.
# Запускается агентом в начале сессии (Startup Workflow) и перед claim'ом "done".
# ЗАПОЛНИТЬ: если ваш стек не определится автоматически — впишите свои команды
# в ветку else в конце файла (или замените весь файл на команды вашего проекта).
...
if [ -f package.json ]; then
  ...
  # ЗАПОЛНИТЬ: при необходимости замените на mvn -q test / mvn verify / свой профиль.
  mvn test
elif [ -f build.gradle ] || [ -f build.gradle.kts ]; then
  echo "=== Running Gradle verification ==="
  # ЗАПОЛНИТЬ: при необходимости замените на ./gradlew check / test / build.
  ./gradlew test
  ...
  echo "ЗАПОЛНИТЬ: замените этот блок на команды верификации вашего проекта"
  echo "(например: mvn test, ./gradlew check, make -f project.mk test)."
fi
